...
- Datastage 5.2.2 Patch 4
- IKC 5.2.2 Connectivity Cumulative Patch
- Pipelines 5.2.2 Hot Fix 1

### Remove Datastage 5.2.2 Patch 4 (manual removal, est. 20 minutes)
...
### IKC 5.2.2 Connectivity Cumulative Patch (manual removal, est. 5 minutes)
...
### Remove Pipelines 5.2.2 Hot Fix 1 (manual removal, est. 15 minutes)
...
### Upgrade WKC custom resource (est. 100 minutes including patching):
...
Confirm WKC pods are in Running/Completed status:
...
### Upgrade IBM Orchestration Pipelines custom resource (est. 35 minutes including patching):
...
Confirm ws_pipeline pods are in Running/Completed status:

```
oc get pods -n ${PROJECT_CPD_INST_OPERANDS} | grep -E "pipeline|workflow|minio"
```

Orchestration Pipelines is upgraded when the install-components command returns:

[SUCCESS]... The install-components command ran successfully


### Upgrade DataStage custom resource (est. 60 minutes including patching):
...
Patch the datastage catalog source:

```
oc patch catsrc -n ${PROJECT_CPD_INST_OPERATORS} ibm-cpd-datastage-ent-plus-operator-catalog --type='json' -p='[{"op": "replace", "path": "/spec/image", "value":"icr.io/cpopen/ds-operator-catalog@sha256:b7a9d36f533cc9aa196ff3eba5c73fc3ece6e60d8a5e06f122bc160f2ceac234"}]'
```
...
```
oc delete csv -n ${PROJECT_CPD_INST_OPERATORS} ibm-cpd-datastage-operator.v8.2.0
```
...
Increase the datastage operator pod memory by updating the memory requests/limits in the CSV:
...
```
oc edit csv ibm-cpd-datastage-operator.v8.2.0
```
...
Confirm DataStage pods are in Running/Completed status:

```
oc get pods -n ${PROJECT_CPD_INST_OPERANDS} | grep datastage
```
