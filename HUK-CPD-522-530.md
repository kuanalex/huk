...
12. Upgrade the configuration admission controller webhook
13. Validate CPD upgrade (customer acceptance test)
```
...
```
oc get sa runtime-manager-api -n cp4d -o yaml
```
...
```
oc describe pod runtime-assemblies-operator-7b6f77f7b9-8qrk4 -n cp4d | grep "Service Account"
```

Check the Service Account
```
oc get sa runtime-assemblies-operator -n cp4d -o yaml
```

Patch it to include the entitlement secret
```
oc patch serviceaccount runtime-assemblies-operator -n cp4d \
...
oc delete pod runtime-manager-api-64bccc9497-nwhgt -n cp4d

# Delete the runtime-assemblies-operator pod
oc delete pod runtime-assemblies-operator-dockercfg-n7f9x -n cp4d
```
...
To apply a new hotfix execute the following commands
...
```
oc patch catsrc -n ${PROJECT_CPD_OPERATORS} ibm-cpd-wspipelines-operator-catalog --type='json' -p='[{"op": "replace", "path": "/spec/image", "value":"icr.io/cpopen/ibm-cpd-wspipelines-operator-catalog@sha256:351caa97d1a1baf873bcb7c6b652881eb2faa808e477ce7d59143680d95cedcf "}]'
```
...
oc delete csv -n ${PROJECT_CPD_OPERATORS} ibm-cpd-wspipelines.v12.0.0
```
...
Wait a few minutes for the CSV to be ready
```
oc -n ${PROJECT_CPD_OPERATORS} get csv ibm-cpd-wspipelines.v12.0.0 -o jsonpath="{.status.phase}"
```
...
```
oc label csv ibm-cpd-wspipelines.v12.0.0 -n $PROJECT_CPD_OPERATORS support.operator.ibm.com/hotfix=true
```

Verify the newly added label
```
oc label csv ibm-cpd-wspipelines.v12.0.0 -n $PROJECT_CPD_OPERATORS support.operator.ibm.com/hotfix=true
```
...
wspipelines-cr   5.3.0     5.3.0        Completed    100%      35m
```
...
```
cpd-cli manage case-download --release=5.3.0 --components=datastage_ent_plus
```
...
global:
  ...
  tetheredNamespaces: []
  blockStorageClass: <your storage class>
  fileStorageClass: <your storage class>
...
```
helm upgrade datastage-ent-plus /root/cpd-cli-workspace/olm-utils-workspace/work/offline/5.3.0/.ibm-pak/data/cases/ibm-datastage/11.0.0/charts/datastage-ent-plus-11.0.0+20251203.100137.3359.tgz -f values-override.yaml
```
...
## 12. Upgrade the configuration admission controller webhook (est. 2 minutes):
